...
const density = '01▏▕▍▐' // estilo hacker, minimal y binario
...
const CHANGE_INTERVAL = 8 // cambiar carácter cada 8 frames (ajustable)

const AsciiTrailDyna = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null)
  const bufferRef = useRef<Float32Array>()
  const charIndexRef = useRef<Uint8Array>()
  ...
  useEffect(() => {
    ...
    charIndexRef.current = new Uint8Array(cols * rows) // para guardar índice actual de carácter

    // Inicializar con índice aleatorio
    for (let i = 0; i < cols * rows; i++) {
      charIndexRef.current[i] = Math.floor(Math.random() * density.length)
    }
    ...
    const draw = () => {
      ...
      // Actualizar índice carácter solo cada CHANGE_INTERVAL frames
      const updateChar = frameCountRef.current % CHANGE_INTERVAL === 0
      ...
      for (let y = 0; y < rows; y++) {
        for (let x = 0; x < cols; x++) {
          ...
          if (v > 0.05) {
            v = Math.min(v, 0.9)
            if (updateChar) {
              charIndex[idx] = Math.floor(Math.random() * density.length)
            }
            const char = density[charIndex[idx]]
